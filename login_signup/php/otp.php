...
if (!empty($otp)) {
    if (strcasecmp($otp, $session_otp) === 0) { // case-insensitive comparison
        $stmt = $pdo->prepare("SELECT * FROM users WHERE unique_id = :unique_id AND otp = :otp");
        ...
        if ($stmt->rowCount() > 0) { //if unique_id and otp match
            $null_otp = 0;
            $stmt2 = $pdo->prepare("UPDATE users SET `verification_status`= 'verified', `otp` = :null_otp WHERE unique_id = :unique_id");
